...
if (!class_exists('WooAdminFixedQuantity')) {
    class WooAdminFixedQuantity
    {
        ...
        public function __construct($file)
        {
            ...
            add_action('wp_ajax_load_woofix_admin_page', array($this, 'load_woofix_admin_page'));
        }
        ...
        public function add_action_links($links)
        {
            ...
            array_unshift($links, '<a href="' . $settingURL . '">' . __('Settings') . '</a>');
        ...
        function load_woofix_admin_page()
        {
            /** @noinspection PhpUnusedLocalVariableInspection */
            $post = get_post($_POST['product_id']);
            if (!$post) {
                _e('No post exist', 'woofix');
            } else {
                /** @noinspection PhpUnusedLocalVariableInspection */
                ...
                /** @noinspection PhpUnusedLocalVariableInspection */
                $woofix_roles = $this->get_woofix_roles();

                /** @noinspection PhpUnusedLocalVariableInspection */
                $woofix_default_role = get_option(WOOFIXOPT_DEFAULT_ROLE, WOOFIXCONF_DEFAULT_ROLE);

                /** @noinspection PhpIncludeInspection */
                include_once plugin_dir_path($this->file) . 'views/html-admin-meta-box-content.php';
            }
            
            wp_die();
        }

        private function get_woofix_roles()
        {
            $woofix_default_role = get_option(WOOFIXOPT_DEFAULT_ROLE, WOOFIXCONF_DEFAULT_ROLE);
